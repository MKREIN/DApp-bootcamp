...
const decorateOrder = (order, tokens) => {
  ...
  if (order.tokenGive === tokens[1].address) {
    ...
    token0Amount = order.amountGet; // The amount of Nox token we want
    token1Amount = order.amountGive; // The amount of Spud token we are giving...
  }
...
const decorateMyFilledOrder = (order, account, tokens) => {
  ...
  let orderType;
  if (myOrder) {
    orderType = order.tokenGive === tokens[1].addresss ? 'buy' : 'sell';
  } else {
    orderType = order.tokenGive === tokens[1].addresss ? 'sell' : 'buy';
  }
